{{--Formulário de edição de pizzas--}}

<div id="menu_edit_pizza" class="hidden">

    @isset($pizzas)
    @foreach ($pizzas as $pizza)
    <div class="mt-20 w-full max-w-xs mx-auto">
        <form class="bg-gray-50 shadow-md rounded px-8 pt-4 pb-4 mb-4" method="POST" action="{{route('update_pizza', [$pizza->id])}}" onsubmit="let pizza_name = '{{$pizza->name}}';let pizza_name_uppercase = pizza_name.toUpperCase();return confirm('Você tem certeza que deseja editar a pizza '+pizza_name_uppercase+'?');">
            @CSRF
            @method("PUT")
            ...
            <div>
                ...
                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 mb-3 leading-tight focus:outline-none focus:shadow-outline text-center" name="price"  type="number" step=".01" value="{{number_format((float)$pizza->price, 2, '.', '')}}">
            </div>

            <div class="flex items-center justify-between">
                <button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline mx-auto text-center" type="submit">
                    Editar Pizza
                </button>
            </div>
        </form>
